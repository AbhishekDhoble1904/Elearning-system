...
"				ele.value='Final Score: "+right+" / "+length+"';"
                    +
"		</script>");
            out.println("</body>");
            out.println("</html>");
            ...
            if(rs.next()){
                ...
                marks = marks+","+right+"/"+length;
                
                pd = c.prepareStatement("update test_history set test_ids=?, marks=? where student_id = ?");
                
                pd.setString(1, test_ids);
                pd.setString(2, marks);
                pd.setString(3, student_id);
